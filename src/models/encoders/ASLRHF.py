...
class SeqEmbedding(nn.Module):
    ...
    def __init__(self, in_channels, num_hid, kernel_size, dilation=1, stride=1, activation='relu', init='he', learn_nan_emb=False, causal_pad=False):
        super().__init__()
        self.learn_nan_emb = learn_nan_emb
        self.causal_pad = causal_pad
        to_pad_fn = lambda seq: (seq.shape[2]*stride)-seq.shape[2]+dilation*kernel_size-dilation+1-stride
        self.pad_fn = lambda seq: F.pad(seq, (to_pad_fn(seq),0)) 
        padding = 'valid' if causal_pad else 'same'

        self.conv1 = nn.Conv1d(in_channels=in_channels, out_channels=num_hid, kernel_size=kernel_size, padding=padding, dilation=dilation, stride=stride)
        self.conv2 = nn.Conv1d(in_channels=num_hid, out_channels=num_hid, kernel_size=kernel_size, padding=padding, dilation=dilation, stride=stride)
        self.conv3 = nn.Conv1d(in_channels=num_hid, out_channels=num_hid, kernel_size=kernel_size, padding=padding, dilation=dilation, stride=stride)
    ...
    def forward(self, seq):
        ...
        seq = self.activation_fn(self.conv1(self.pad_fn(seq) if self.causal_pad else seq))
        seq = self.activation_fn(self.conv2(self.pad_fn(seq) if self.causal_pad else seq))
        seq = self.activation_fn(self.conv3(self.pad_fn(seq) if self.causal_pad else seq))
